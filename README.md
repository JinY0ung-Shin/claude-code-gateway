...
| `CLAUDE_MAX_BUFFER_SIZE` | Byte limit for ONE JSON message on the Claude CLI's stdout — a tool result is one message, so anything larger fails the whole turn with `sdk_error` (fatal SDK reader abort, not a tool error). The gateway installs its own `16777216` (16 MiB) default instead of the SDK's 1 MiB; set explicitly to raise or lower it (a value at or below 1 MiB is a startup warning) |
